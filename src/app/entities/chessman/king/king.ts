...
export class King extends Chessman {
  ...
  move(boardData: Record<string, Position>, currentPosition: string, histories?: BoardHistories): Array<string> {
    ...
    // castle
    if (histories[histories.length - 1]?.isCheckmate) return moveContainer;
    if (boardData[currentPosition].object.get().side) {
      if (boardData["e1"].object.get().code === "K" && boardData["h1"].object.get().code === "R")
        if (!histories.find((history) => ["e1", "h1"].includes(history.move.slice(0, 2))))
      ...
      if (boardData["e1"].object.get().code === "K" && boardData["a1"].object.get().code === "R")
        if (!histories.find((history) => ["e1", "a1"].includes(history.move.slice(0, 2))))
      ...
      if (boardData["e8"].object.get().code === "k" && boardData["h8"].object.get().code === "r")
        if (!histories.find((history) => ["e8", "h8"].includes(history.move.slice(0, 2))))
      ...
      if (boardData["e8"].object.get().code === "k" && boardData["a8"].object.get().code === "r")
        if (!histories.find((history) => ["e8", "a8"].includes(history.move.slice(0, 2))))
